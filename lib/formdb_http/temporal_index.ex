...
defmodule FormdbHttp.TemporalIndex do
  @moduledoc """
  ...
  def handle_call({:create_index, db_id, series_id}, _from, state) do
    table_name = table_name(db_id, series_id)

    case :ets.info(table_name) do
      :undefined ->
        # Create ordered_set table for efficient range queries
        :ets.new(table_name, [:ordered_set, :named_table, :public, {:write_concurrency, true}])
        new_indexes = Map.put(state.indexes, {db_id, series_id}, table_name)
  ...
  def handle_call({:insert, db_id, series_id, point_id, timestamp_unix}, _from, state) do
    ...
    case :ets.info(table_name) do
      ...
      _ ->
        ...
        :ets.insert(table_name, {{timestamp_unix, point_id}, %{
          point_id: point_id,
  ...
  def handle_call({:range_query, db_id, series_id, start_unix, end_unix, limit}, _from, state) do
    ...
    case :ets.info(table_name) do
      ...
      _ ->
        ...
        results = range_select(table_name, start_unix, end_unix, limit)
  ...
  def handle_call({:delete, db_id, series_id, point_id, timestamp_unix}, _from, state) do
    ...
    case :ets.info(table_name) do
      ...
      _ ->
        :ets.delete(table_name, {timestamp_unix, point_id})
        {:reply, :ok, state}
    end
  end
  ...
  def handle_call({:stats, db_id, series_id}, _from, state) do
    ...
    case :ets.info(table_name) do
      ...
      _ ->
        ...
        {min_ts, max_ts} =
          case {:ets.first(table_name), :ets.last(table_name)} do
  ...
  defp table_name(db_id, series_id) do
    # Create atom from db_id and series_id (sanitize for atom creation)
    ...
    String.to_atom("#{@table_prefix}#{db_hash}_#{series_hash}")
  end
